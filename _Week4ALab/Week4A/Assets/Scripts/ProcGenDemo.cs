...
public class ProcGenDemo : MonoBehaviour {
	...
	public Transform prefab; // assign in INspecor
	public List<Transform> listOfClones =  new List<Transform>();

	// Use this for initialization
	void Start () {

		// a FOR LOOP is a WHILE LOOP with built-in control
		// 1st part: initialize a variable, usually a counter
		//2nd part: the "if" statement to check each time
		// 3rd part: waht to do at the end. usually incement counter
		for ( int i=0;  i < 200; i++) {
			// "CASTING" is telling Unity that one type of variable is actually another typemof variable
